optimizacion.punto.corte.ROC.MinValueSp.lm <- function (modelos = lista.modelos,minSp=0.9) { #funcion que busca el punto de corte optimo donde la especificidad es maxima y se cumplen con los criterios de la minima especificidad(minSp) y minima sensibilidad(minSe) que yo seteo
        ...
        lista.predicciones <- lapply( modelos , predict , type = "response") #armo lista donde cada objeto es un modelo y tiene los valores predichos para cada compuesto
        ...
        for (i in 1:length(resumen.punto.corte)) { #loop para extraer los datos que me interesan del resultado del anterior loop
                list.df[[i]]<-as.data.frame(resumen.punto.corte[[i]]$p.table$Global$MinValueSp) #extraigo los datos que me interesan para cada modelo
...
puntos.corte.ROC.lm <-optimizacion.punto.corte.ROC.MinValueSp.lm(modelos = lista.modelos, minSp=0.9)
