...
forward.stepwise.testF.lm <- function(tabla.conjunto , punto.corte=0.05 , steps=6 , punto.corte.vif = 2) { # funcion para realizar los modelos por lm
        ...
        if ( is.installed("faraway") == FALSE) {install.packages("faraway")} #si car no está instalado hago que me lo instale automaticamente
        
        library(faraway) #cargo el paquete car
