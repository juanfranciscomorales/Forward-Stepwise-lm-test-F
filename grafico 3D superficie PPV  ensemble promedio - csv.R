clasificaciones.test.set.ensemble.promedio.lm2 <- function (test.set = "dude.csv",cant.modelos = 10, x = tabla.AUC.ordenadas) {
        ...
        promedio<-apply(tabla.valores.prediccion.test,1,mean, na.rm=FALSE)#aplico operador promedio en los valores predichos de los mejores modelos para cada compuesto
        
        clase <-df.test.set[,"clase" ] #extraigo los valores de la columna clase
        
        resultado <- cbind(promedio,clase)
        
        colnames(resultado)<- c("promedio", "clase")
...
### Y EL VALOR promedio PARA CADA COMPUESTO DE LA BASE DUDE

df <- clasificaciones.test.set.ensemble.promedio.lm2(test.set  = "Dudepoliaminas2016.csv",cant.modelos = 8, x = tabla.AUC.ordenadas.dude)  ## aplico la funcion anterior para obtener un data frame con los valores de score promedio y la clase real
...
curva.roc.dude <- roc(predictor = df$promedio ,response = df$clase, direction="<" , ci = TRUE , auc = TRUE , conf.level=0.95 , ci.method = "delong", boot.n = 2000, boot.stratified = TRUE, reuse.auc=TRUE , plot =  TRUE) ## calculo la curva ROC con los datos del score minimo y la clase verdadera
...
predicciones <- ifelse( df$promedio > resultados.ensemble.promedio[[8]], yes = 1,no = 0) ## predicciones aplicando el ensemble de operador promedio y usando el punto de corte que obtuve con el training
...
curva.ROC.dude <- roc(response = df$clase, predictor = df$promedio, direction = "<") ## calculo de la curva ROC para los resultados de la base dude
...
for( j in secuencia) {
        ...
        htmlwidgets::saveWidget(as.widget(p), file =  paste("GRAFICO 3D PPV ", j ," MEJORES MODELOS ENSEMBLE PROMEDIO", ".html", sep="")) ### GUARDO EL GRÁFICO COMO HTML Y LUEGO LO PUEDO VER EN CUALQUIER NAVEGADOR WEB
