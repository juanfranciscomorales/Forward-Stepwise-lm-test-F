...
clasificaciones.test.set.ensemble.minimo.lm <- function (test.set = "Dtest.csv",cant.modelos = 10, x = tabla.AUC.ordenadas, remover.NA = FALSE, graficar = TRUE){
        ...
        ROC.ensemble.minimo <- roc(predictor = minimo, response = clase, direction = "<" , ci = TRUE , auc = TRUE , conf.level=0.95 , ci.method = "delong", boot.n = 2000, boot.stratified = TRUE, reuse.auc=TRUE, plot = graficar , print.auc = TRUE, main = "Test set - lm - Ensemble Minimo")#creo lista donde voy a guardar las curvas ROC
        ...
        grafico <- ggplot () + 
                ...
                ggtitle("Test set - lm - Ensemble Minimo") +
...
clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.AUC.ordenadas, remover.NA = FALSE, graficar = TRUE)

clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.AUC.ordenadas.test.set, remover.NA = FALSE, graficar = TRUE)

clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.sensibilidad.ordenadas, remover.NA = FALSE, graficar = TRUE)

clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.AUC.ordenadas.dude, remover.NA = FALSE, graficar = TRUE)

clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.porcentajes.buenas.clasificaciones.k.fold.CV.lm, remover.NA = FALSE, graficar = TRUE)

clasificaciones.test.set.ensemble.minimo.lm(test.set = "Dtest.csv",cant.modelos = 10, x = tabla.AUROC.k.fold.CV.lm, remover.NA = FALSE, graficar = TRUE)
