optimizacion.punto.corte.ROC.test.set.MinValueSp.lm <- function (test.set = "BASE SIMULADA - Poliaminas.csv", modelos = lista.modelos,minSp=0.9) { #funcion que busca el punto de corte optimo donde la especificidad es maxima y se cumplen con los criterios de la minima especificidad(minSp) y minima sensibilidad(minSe) que yo seteo
