optimizacion.punto.corte.ROC.MinValueSpSe.lm <- function (modelos = lista.modelos,minSp=0.9,minSe=0.9) { #funcion que busca el punto de corte optimo donde la especificidad es maxima y se cumplen con los criterios de la minima especificidad(minSp) y minima sensibilidad(minSe) que yo seteo
...
puntos.corte.ROC.lm <- optimizacion.punto.corte.ROC.MinValueSpSe.lm(modelos = lista.modelos,minSp=0.9,minSe=0.9 )
