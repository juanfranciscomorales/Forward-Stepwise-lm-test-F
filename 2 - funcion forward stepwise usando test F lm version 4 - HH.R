...
forward.stepwise.testF.lm <- function(tabla.conjunto , punto.corte=0.05 , steps=6 , punto.corte.vif = 2) { # funcion para realizar los modelos por lm
        ...
        if ( is.installed("HH") == FALSE) {install.packages("HH")} #si car no está instalado hago que me lo instale automaticamente
        
        library(HH) #cargo el paquete car
        
        df <- as.data.frame(tabla.conjunto)#hago por las dudas que la tabla del conjunto me la lleve a clase data frame
        
        null <- lm(clase ~1,data = df, na.action = na.omit , x = TRUE)#modelo inicial para hacer el forward. na.omit hace que se omitan las filas que tienen NA values
        
        full <- lm(clase~.,data = df, na.action = na.omit , x = TRUE)#este es el modelo con todos los descriptores, lo hago para poder armar el scope de la funcion add1.na.omit hace que se omitan las filas que tienen NA values 
