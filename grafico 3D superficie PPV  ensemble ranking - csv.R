...
df <- clasificaciones.test.set.ensemble.ranking.lm2(test.set  = "Dudepoliaminas2016.csv",cant.modelos = 15, x = tabla.AUC.ordenadas.dude)  ## aplico la funcion anterior para obtener un data frame con los valores de score  voto promedio y la clase real
...
predicciones <- ifelse( df$promedio.ranking < resultados.ensemble.ranking[[8]], yes = 1,no = 0) ## predicciones aplicando el ensemble de operador voto y usando el punto de corte que obtuve con el training
...
for( j in secuencia) {
        ...
        htmlwidgets::saveWidget(as.widget(p), file =  paste("GRAFICO 3D PPV " , j," MEJORES MODELOS ENSEMBLE RANKING", ".html", sep="")) ### GUARDO EL GRÁFICO COMO HTML Y LUEGO LO PUEDO VER EN CUALQUIER NAVEGADOR WEB
