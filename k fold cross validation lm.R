...
lista.porcentajes.buenas.clasificaciones.k.fold.CV.lm <- lapply(lista.modelos , k.fold.CV.lm , punto.corte.optimizado = 0.5 , archivo.con.datos = "S-M-H training set lm.csv", folds = 10 , repeticiones = 5)# aplico a cada conjunto la funci?n llamada forward.stepwise.testF.lm para obtener los modelos. si quiero modifico punto de corte y numero de steps
...
write.xlsx(x= tabla.porcentajes.buenas.clasificaciones.k.fold.CV.lm, file= "Resultado k fold CV - lm.xlsx" , colNames= TRUE, keepNA=TRUE) # funcion para guardar los resultados del screening en la base de datos
