...
combinacion.modelos.promedio <- function (x = tabla.AUC.ordenadas, cant.modelos = 10, minSp = 0.98, remover.NA =FALSE) { #funcion donde x es el resultado de la funcion del script de curvas ROC.R y cant.modelos es la cant de modelos que quiero combinar
        ...
        punto.corte <- OptimalCutpoints::optimal.cutpoints (X = "promedio",status="clase",tag.healthy=0, data=df,methods="MinValueSp", control = controles) ## optimizo el punto de corte usando la funcion de maxima especificidad
...
resultados.ensemble.promedio <- combinacion.modelos.promedio(x = tabla.AUC.ordenadas, cant.modelos = 10, minSp = 0.98, remover.NA =FALSE)

resultados.ensemble.promedio <- combinacion.modelos.promedio(x = tabla.sensibilidad.ordenadas, cant.modelos = 10, minSp = 0.98, remover.NA =FALSE)