optimizacion.punto.corte.ROC.max.especificidad <- function (modelos = lista.modelos) { #funcion que busca el punto de corte optimo donde la especificidad es maxima
  ...
  for (i in 1:length(resumen.punto.corte)) { #loop para extraer los datos que me interesan del resultado del anterior loop
    list.df[[i]]<-as.data.frame(resumen.punto.corte[[i]]$p.table$Global$MaxSp) #extraigo los datos que me interesan para cada modelo
...
puntos.corte.ROC.lm <- optimizacion.punto.corte.ROC.max.especificidad(modelos = lista.modelos)
