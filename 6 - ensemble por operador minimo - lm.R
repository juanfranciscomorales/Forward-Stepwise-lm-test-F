...
combinacion.modelos.minimo <- function (x = tabla.AUC.ordenadas, cant.modelos = 10, remover.NA = FALSE , graficar = TRUE) { #funcion donde x es el resultado de la funcion del script de curvas ROC.R y cant.modelos es la cant de modelos que quiero combinar
        ...
        ROC.ensemble.minimo <- roc(predictor = minimo, response = clase , direction = "<" , ci = TRUE , auc = TRUE , conf.level=0.95 , ci.method = "delong", boot.n = 2000, boot.stratified = TRUE, reuse.auc=TRUE , plot = graficar , print.auc = TRUE , main = "Training set - lm - Ensemble Minimo")#creo lista donde voy a guardar las curvas ROC
        ...
        punto.corte <- OptimalCutpoints::optimal.cutpoints (X = "minimo",status="clase",tag.healthy=0, data=df,methods="MaxSp", direction = "<") ## optimizo el punto de corte usando la funcion de maxima especificidad
       ...
       if (graficar == TRUE) {
               ...
               predicciones <- prediction(predictions = minimo, labels = clase) ## genero los valores para armar los diferentes graficos
               ...
               grafico <- ggplot () + 
                       ...
                       ggtitle("Training set - lm - Ensemble Minimo") +
...
resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.AUC.ordenadas, cant.modelos = 10, remover.NA = FALSE , graficar = TRUE)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.AUC.ordenadas.test.set, cant.modelos = 10, remover.NA = FALSE, graficar = TRUE)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.AUC.ordenadas.dude, cant.modelos = 10, remover.NA = FALSE, graficar = TRUE)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.sensibilidad.ordenadas, cant.modelos = 10, remover.NA = FALSE, graficar = TRUE)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.porcentajes.buenas.clasificaciones.k.fold.CV.lm, cant.modelos = 10, remover.NA = FALSE, graficar = TRUE)

resultados.ensemble.minimo <- combinacion.modelos.minimo(x = tabla.AUROC.k.fold.CV.lm, cant.modelos = 10, remover.NA = FALSE, graficar = TRUE)